...
def evaluate_one(filename):
    ...
    # --- Two data sets ---
    T_cross_1 = data["T_cross_values"]
    T_bar_1   = data["T_bar_values"]
...
def get_values():
    ...
    width_span=np.linspace(width_ridge-0.5e-6,width_ridge+0.5e-6,3)
    for width in width_span:
        dist=width_ridge/3-taper_width
        delta_y=dist/2+taper_width/2-width/6
        # delta_y=0        
    ...
    np.savez(f"data/eme_width_span_{round(width_ridge*1e6,1)}",
        Span=width_span,
...
if __name__=="__main__":
    # evaluate_one("data/eme_width_span_19.npz")
    get_values()
