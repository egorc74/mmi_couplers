    ...
    try:
        ...
        for kkk, taper_val in enumerate(taper_width_span):
            ...
            for lll, mmi_width in enumerate(mmi_width_span):
                ...
                transmission_matrix[kkk, lll] = np.abs(s41[0])**2
...
def taper_width_colormap_3db(
    ...
    Sweep taper width and MMI width deviations to build transmission matrix.
    """
    log.info(f"Starting taper_width_colormap_3db() \n data_filename: {data_filename} \n width_ridge: {width_ridge}, \n mmi_length:{mmi_length},\n taper_width={taper_width},\n delta_y:{delta_y}")
    try:
        # Parameters for sweep
        span = 0.5e-6
        taper_width_span = np.linspace(taper_width_in - span, taper_width_in + span, 11)
        mmi_width_span = np.linspace(width_ridge - span, width_ridge + span, 11)
        log.info(f" In taper_width_colormap_3db() width_span {mmi_width_span},"
                    f"with taper span {taper_width_span}")
        #0 index for 4 monitor; 1 index for 3 monitor
        transmission_matrix = np.zeros((len(taper_width_span), len(mmi_width_span),2))  
        ...
        for kkk, taper_val in enumerate(taper_width_span):
            ...
            for mm in range(num_p):
                opt_length = start_len + mm * length_dx
                trans_4_1 = np.abs(s41[mm])**2
                trans_3_1=  np.abs(s31[mm])**2
                if trans_4_1 > max_trans_4_1:
                    max_trans_4_1 = trans_4_1
                    max_length_4_1 = opt_length
                if trans_3_1 > max_trans_3_1:
                    max_trans_3_1 = trans_3_1
                    max_length_3_1 = opt_length
                    
            log.info(f"In taper_width_colormap obtained for 4:{max_trans_4_1},"
                            f"optimal_length for 4 monitor: {max_length_4_1} for width_ridge{width_ridge}"
                            f"In taper_width_colormap obtained for 3:{max_trans_3_1},"
                            f"optimal_length for 3 monitor: {max_length_3_1} for width_ridge{width_ridge}")
            mmi_length = (max_length_3_1+max_length_4_1)/2   #mean value 
            log.info(f"optimal length selected as mean value{mmi_length}")
            ...
            for lll, mmi_width in enumerate(mmi_width_span):
                ...
                transmission_matrix[kkk, lll,0] = np.abs(s41[0])**2
                transmission_matrix[kkk, lll,1] = np.abs(s31[0])**2
...
if __name__=="__main__":
    ...
    if os.path.isfile(f"{filename}.lms"):
        ...
        sim=lumapi.MODE()
    colormap_transmission_matrix,colormap_taper_width_span,colormap_mmi_width_span=taper_width_colormap_3db(sim=sim,filename=filename,data_filename=data_filename,
                                                                                    width_ridge=width_ridge,
