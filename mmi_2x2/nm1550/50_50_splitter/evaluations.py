...
def colormap_evaluation_3db(transmission_matrix,colormap_taper_span,colormap_width_span,plot=None):
    try:
        # ---------------------------
        # Example data setup
        # ---------------------------
        ...
        log.info(f"starting colormap_evaluation_3db() with transmision matrix shape{transmission_matrix.shape} "
                    f", colormap_taper_span: {colormap_taper_span[:3]} ... {colormap_taper_span[-3:]}"
                    f", colormap_width_span: {colormap_width_span[:3]} ... {colormap_width_span[-3:]}")


        # Compute deviation matrix
        deviation_taper_width_matrix = np.zeros((len(x), 2))
        
        # Compute average difference of port transmissions
        delta_transmission=transmission_matrix[:,:,0]-transmission_matrix[:,:,1]
        avg_delta_transmission=np.zeros((len(x)))
        ##first collumn taper   second collumn width ridge. for every taper calculate its t at [ii,5] 
        
        for ii in range(len(x)):
            max_val = transmission_matrix[ii, 5,0]   # MATLAB's Z(ii,6) → Python is 0-based
            avg_all = np.mean(max_val - transmission_matrix[ii, :,0])
            deviation_taper_width_matrix[ii, 0] = max_val - avg_all
            deviation_taper_width_matrix[ii, 1] = avg_all
            avg_delta_transmission[ii]=np.mean(delta_transmission[ii,:])
        ...
        if plot!=None:
            ...
            fig2.add_trace(
                go.Scatter(
                    x=x, y=avg_delta_transmission,
                    mode="lines", line=dict(color="black"),
                    name="Average output difference"
                ),
                secondary_y=True,
            )
...
if __name__=="__main__":
    ...
    data_filename="data/taper_width_colormap_3db_2500_11000nm.npy"
    transmission_matrix = np.load(data_filename, allow_pickle=True)
    print(transmission_matrix[10,5,0])

    width_span=np.linspace(10.5,11.5,11)*1e-6
    taper_span=np.linspace(2,3,11)*1e-6
    colormap_evaluation_3db(transmission_matrix=transmission_matrix,colormap_taper_span=taper_span,colormap_width_span=width_span,plot=1)
