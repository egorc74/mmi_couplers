...
def taper_geometry(sim, filename,taper_width,taper_length,straight_section=False):
    ...
    if straight_section:
        ...
        sim.set("z min",-thick_Si3N4/2)     
        sim.set("z max", thick_Si3N4/2)
        sim.set("x min",taper_length/2)  
        sim.set("x max",taper_length*3/2)
...
if __name__=="__main__":
    ...
    taper_length=50e-6
    taper_width=3e-6
    taper_geometry(sim=lumapi.MODE(),filename=filename,taper_length=taper_length,taper_width=taper_width,straight_section=True)
