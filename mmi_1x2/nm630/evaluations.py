    ...
    try:
        ...
        if plot!=None:
            fig1 = px.imshow(
                ...
                labels={"x": "W input", "y": "W mmi", "color": "Transmission"},
                title=f"Color-coded map of Transmission for wavelength:{wavelength},"
            ...
            fig2.add_trace(
                go.Scatter(
                    ...
                    mode="lines+markers", name="Taper width deviation"
                ),
                secondary_y=False,
            )
...
if __name__=="__main__":
    ...
    # data_filename="data/core_sweep_from_7500nm_to_10500nm_with_taper_3000nm.npy"
    ...
    data_filename="data/taper_width_colormap_2600_8300nm.npy"
    transmission_matrix = np.load(data_filename, allow_pickle=True)
        

    width_span=np.linspace(7.8,8.8,11)*1e-6
    taper_span=np.linspace(2.1,3.1,11)*1e-6
    ...
    data_filename="data/taper_width_colormap_4400_11000nm.npy"
    transmission_matrix = np.load(data_filename, allow_pickle=True)
        

    width_span=np.linspace(10.5,11.5,11)*1e-6
    taper_span=np.linspace(3.9,4.9,11)*1e-6
    
    colormap_evaluation(transmission_matrix=transmission_matrix,colormap_taper_span=taper_span,colormap_width_span=width_span,plot=1)
    
    
    data_filename="data/taper_width_colormap_3500_9200nm.npy"
    transmission_matrix = np.load(data_filename, allow_pickle=True)
        

    width_span=np.linspace(8.7,9.7,11)*1e-6
